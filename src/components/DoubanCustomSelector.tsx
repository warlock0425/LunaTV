...
const DoubanCustomSelector: React.FC<DoubanCustomSelectorProps> = ({
  ...
  const renderCapsuleSelector = (
    ...
    return (
      <div
        ref={containerRef}
        className='relative inline-flex bg-zinc-200/60 rounded-full p-0.5 sm:p-1 dark:bg-zinc-700/60 backdrop-blur-sm'
      >
        {/* 滑動的白色背景指示器 */}
        {indicatorStyle.width > 0 && (
          <div
            className='absolute top-0.5 bottom-0.5 sm:top-1 sm:bottom-1 bg-white dark:bg-zinc-500 rounded-full shadow-sm transition-all duration-300 ease-out'
            style={{
            ...
            <button
              ...
              className={`relative z-10 px-2 py-1 sm:px-4 sm:py-2 text-xs sm:text-sm font-medium rounded-full transition-all duration-200 whitespace-nowrap ${
                isActive
                  ...
                  : 'text-zinc-700 hover:text-zinc-900 dark:text-zinc-400 dark:hover:text-zinc-100 cursor-pointer'
              }`}
            >
              {option.label}
            </button>
    ...
    <div className='space-y-4 sm:space-y-6'>
      ...
      <div className='space-y-3 sm:space-y-4'>
        ...
        <div className='flex flex-col sm:flex-row sm:items-center gap-2'>
          <span className='text-xs sm:text-sm font-medium text-zinc-600 dark:text-zinc-400 min-w-[48px]'>
            類型
          </span>
